module SettingsHelper
  def competition_started?
    ...
    setting = Setting.find_by(name: 'start_time')
    return false unless setting
    start_time = DateTime.strptime(setting.value + offset, '%m/%d/%Y %I:%M %p %Z').in_time_zone
    DateTime.current.to_i > start_time.to_i
  end
  
  def competition_ended?
    offset = " #{DateTime.current.formatted_offset(false)}"
    setting = Setting.find_by(name: 'end_time')
    return false unless setting
    end_time = DateTime.strptime(setting.value + offset, '%m/%d/%Y %I:%M %p %Z').in_time_zone
  ...
  def competition_name
    Setting.find_by(name: 'competition_name')&.value || "TrackForge"
  end
  ...
  def entry_cost
    Setting.find_by(name: 'entry_cost')&.value || "0"
  end
